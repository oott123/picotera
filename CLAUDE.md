...
- **Project matching**: every gateway request body is scanned by `pkg/server/project_extractor.go` against four fixed regexes (`Workspace root folder:`, `Primary working directory:`, `Current working directory:`, `<cwd>…</cwd>`). Captures are JSON-string unescaped to produce candidate paths. Candidates are extracted early (pure, no DB); resolution to a `project_id` runs **after authentication** because projects are user-bound — the lookup is scoped to the api_key's `account_id`. `Server.projectRouter` is an in-memory longest-prefix cache from `db/queries/project.sql:ListProjectPaths`, **bucketed by `account_id`** (the `endpointRouter` stays globally keyed). On first-unseen-cwd-per-account the gateway auto-creates a project under the caller's account (`name = filepath.Base(path)`, fallback `untitled`; `ON CONFLICT (account_id, name) DO NOTHING` handles concurrent inserts) and emits a `auth.project_auto_created` log line. The matched `project_id` is backfilled onto the meta row via `UpdateRequestProjectID` and propagates to each upstream attempt; an asynchronous `UpsertProjectSeen` updates `project.first_seen_at` / `project.last_seen_at`. System keys (`api_key.account_id IS NULL`) never tag a project and never auto-create. Any mutation of the `project` table **must** call `Server.projectRouter.Invalidate()`.
...
Thirteen tables: `provider`, `endpoint`, `provider_endpoint`, `model`, `model_provider_endpoint`, `api_key`, `request` (hypertable), `script`, `traces`, `project`, `account`, `webauthn_credential`, `enrollment`. Uses JSONB for flexible fields (provider models, annotations, project paths). Upsert pattern via `ON CONFLICT DO UPDATE`. The `request` hypertable also carries a nullable `project_id` foreign reference (no FK constraint) populated by the project extractor after authentication. The `project` table is **per-user** since migration 029: `account_id NOT NULL` (FK to `account`, `ON DELETE CASCADE`), uniqueness on `(account_id, name)` (two users may have projects with the same name without collision).
