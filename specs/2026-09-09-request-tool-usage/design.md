...
扫描一张固定的候选路径表 `["tool_usage", "response.tool_usage"]`，先命中先用 —— 两条路径覆盖全部四种
上游格式，一份代码即可。

### 落库形状

归一化成数组，key 变成 `name`，只保留四个白名单整数字段：
...
`extractToolUsage(result gjson.Result)` 按 `toolUsagePaths` 找到 `tool_usage` 对象，再按上面的规则
产出条目数组。**最后一次非空出现覆盖之前的值**（last wins），与 `usage` 字段既有的覆盖语义一致。
...
| 调用点 | 覆盖场景 |
|---|---|
| `processSSEEvent` | 全部四种 SSE 格式（两条候选路径覆盖） |
...
`selected.toolUsage?.length`。每个工具一行 `Field`，label 是工具名，值是该工具报出的字段拼成的紧凑摘要
（例如 `请求 1`、`输入 222 · 输出 1630`），沿用现有 2 列 grid 与 `fmtNum`。
